...
function getKeysFromObject(obj: any, prefix = ''): string[] {
  const keys: string[] = [];
...
describe('Translation Keys Consistency', () => {
  const jsonFiles = getAllJsonFiles(join(LOCALES_DIR, SUPPORTED_LOCALES[0], 'pages'));
  ...
  describe.each(jsonFiles)('Translation file: %s', (filename) => {
    it(`should have matching keys between all locales for ${filename}`, () => {
      ...
      for (const locale of otherLocales) {
        ...
        expect(missingKeys, `Missing keys in ${locale} (${filename}): ${missingKeys.join(', ')}`).toHaveLength(0);
        expect(extraKeys, `Extra keys in ${locale} (${filename}): ${extraKeys.join(', ')}`).toHaveLength(0);
        expect(localeKeys, `Keys mismatch between ${baseLocale} and ${locale} for ${filename}`).toEqual(baseKeys);
      }
    });
  });
  ...
  it('should have the same translation files in all locales', () => {
    ...
    for (const locale of otherLocales) {
      ...
      expect(missingFiles, `Missing translation files in ${locale}: ${missingFiles.join(', ')}`).toHaveLength(0);
      expect(extraFiles, `Extra translation files in ${locale}: ${extraFiles.join(', ')}`).toHaveLength(0);
      expect(localeFiles, `Translation files mismatch between ${baseLocale} and ${locale}`).toEqual(baseFiles);
    }
  });
});